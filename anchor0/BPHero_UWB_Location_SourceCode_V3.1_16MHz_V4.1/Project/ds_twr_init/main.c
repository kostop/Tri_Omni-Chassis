...
#define POLL_RX_TO_RESP_TX_DLY_UUS 7400
/* This is the delay from the end of the frame transmission to the enable of the receiver, as programmed for the DW1000's wait for response feature. */
#define RESP_TX_TO_FINAL_RX_DLY_UUS 800
...
#define RESP_RX_TO_FINAL_TX_DLY_UUS 6200 //2700 will fail
...
car_measure_t target_car_measure[CAR_NUM];
car_measure_t car_measure[CAR_NUM];
...
controller_rx_t controller_rx;
uint8_t controller_rx_date[17];
...
/*
	...
	遥控器 C8T6-->>UWB C8T6
*/
static uint8_t receive_buffer=0;
...
void USART1_IRQHandler()
{
	if(USART_GetITStatus(USART1,USART_IT_RXNE)==SET)
	{
        ...
        if(receive_buffer==0xf5&&receive_state!=0x03)
        {
            receive_state = 0;
        }
        ...
        {
            case 0:
                if(receive_buffer==0x5f)
                    receive_state = 1;
                else
                    receive_state = 0;
                break;
            case 1:
                receive_state = receive_buffer;
                receive_state = 2;
                break;
            case 2:
                switch(receive_state)
                {
                    ...
                    case CAR_NORMAL_MOVE:
                        ...
                        {
                            case 0x1a:
                                target_car_measure->x = 0.5f;
                                break;
                            case 0x1b:
                                target_car_measure->x = -0.5f;
                                break;
                            case 0x1c:
                                target_car_measure->yaw = 0.5f;
                                break;
                            case 0x1d:
                                target_car_measure->yaw = -0.5f;
                                break;
                            case 0xfa:
                                target_car_measure->x = 0.0f;
                                break;
                            case 0xfb:
                                target_car_measure->x = 0.0f;
                                break;
                            case 0xfc:
                                target_car_measure->yaw = 0.0f;
                                break;
                            case 0xfd:
                                target_car_measure->yaw = 0.0f;
                                break;
                    ...
                    case CAR_POSITION_MOVE:
                        ...
                        // pos_set.y = 3.0f;
                        // pos_set.yaw = 0.0f;
                        target_car_measure->state = CAR_POSITION_MOVE;
                        target_car_measure->x = 1.5f;
                        target_car_measure->y = 3.0f;
                        target_car_measure->yaw = 0.0f;
                        break;
                }
                receive_state = 0;
        ...
        //旧版使用C8T6中间通信用的代码
		// controller_rx_date[receive_count++] = receive_buffer;
		// switch(receive_state)
		// {
		// 	case 0:
		...
		// 		break;
		// 	case 1:
		// 		if(receive_buffesr == 0x12)
		// 			receive_state = 2;
		// 		else
		// 			receive_state = 9;
		// 		break;
		// 	case 2:
		// 		if(receive_buffer == 0x5E&&receive_count==17)
		// 			receive_state = 3;
		// 		else if(receive_count>17)
		// 			receive_state = 9;
		// 		break;
		// }
		// if(receive_state == 3)
		// {
		// 	receive_state = 9;
		// 	memcpy(&controller_rx, controller_rx_date, sizeof(controller_rx_date));
		// 	if(controller_rx.id==0x0f)
		// 		controller_rx.id = 0;
		// 	target_car_measure[controller_rx.id].state = controller_rx.state;
		// 	target_car_measure[controller_rx.id].x = controller_rx.x;
		// 	target_car_measure[controller_rx.id].y = controller_rx.y;
		// 	target_car_measure[controller_rx.id].yaw = controller_rx.yaw;
		// }
		// if(receive_state == 9)
		// {
		// 	receive_state = 0;
...
//	{
//		if(i==0)
//			controller_msg[3] = 0x0F;
//		else
//			controller_msg[3] = i;
////		target_car_measure[i].x	= 1.0f;
////		target_car_measure[i].y	= 1.0f;
////		target_car_measure[i].yaw	= 1.15f;

//		//ң�������Ϳ������ݸ�׮��tag
//		controller_msg[10] = target_car_measure[i].state;
//		controller_msg[11] = ((int16_t)(target_car_measure[i].x*100))>>8;
//		controller_msg[12] = ((int16_t)(target_car_measure[i].x*100));
//		controller_msg[13] = ((int16_t)(target_car_measure[i].y*100))>>8;
//		controller_msg[14] = ((int16_t)(target_car_measure[i].y*100));
//		controller_msg[15] = ((int16_t)(target_car_measure[i].yaw*100))>>8;
//		controller_msg[16] = ((int16_t)(target_car_measure[i].yaw*100));
//		
//        dwt_writetxdata(sizeof(controller_msg), controller_msg, 0);
//        dwt_writetxfctrl(sizeof(controller_msg), 0);
...
//            }
//			if((rx_buffer[3] == 0x0F&&i==0)||rx_buffer[3] == i)
//			{
//				rx_buffer[2] = 0;
//				rx_buffer[3] = 0;   
//				//ң����dw1000���յ�׮��tag����
//				if (memcmp(rx_buffer, car_msg, ALL_MSG_COMMON_LEN) == 0)
//				{
//					car_measure[i].x = (float)((rx_buffer[11]<<8)|rx_buffer[12])/100.0f;
//					car_measure[i].y = (float)((rx_buffer[13]<<8)|rx_buffer[14])/100.0f;
//					car_measure[i].yaw = (float)((rx_buffer[15]<<8)|rx_buffer[16])/100.0f;
//				}
//			}
//		}
//	}
...
/*
	...
	�������ݰ�	target_car_measure[tag_id]
	�������ݰ�	car_measure[tag_id]
*/
			//���жϽ�������
			else if (memcmp(rx_buffer, car_msg, ALL_MSG_COMMON_LEN) == 0)
			{
				//��һ��id�ŷ����������
				if(tag_index == 0x0f)
					tag_index1 = 0x00;
				else
					tag_index1 = tag_index;
//				car_measure[tag_index1].x = (float)((rx_buffer[11]<<8)|rx_buffer[12])/100.0f;
//				car_measure[tag_index1].y = (float)((rx_buffer[13]<<8)|rx_buffer[14])/100.0f;
//				car_measure[tag_index1].yaw = (float)((rx_buffer[15]<<8)|rx_buffer[16])/100.0f;
				
				car_measure[tag_index1].x = parseSignedFloat(rx_buffer[11], rx_buffer[12], 100.0f);
				car_measure[tag_index1].y = parseSignedFloat(rx_buffer[13], rx_buffer[14], 100.0f);
				car_measure[tag_index1].yaw = parseSignedFloat(rx_buffer[15], rx_buffer[16], 100.0f);
				
				dwt_setrxaftertxdelay(POLL_RX_TO_RESP_TX_DLY_UUS);
				dwt_setrxtimeout(FINAL_RX_TIMEOUT_UUS);
				
				//��������׮���Ŀ�������
				controller_msg[3] = tag_index;
				controller_msg[10] = target_car_measure[tag_index1].state;
				floatToSignedBytes(target_car_measure[tag_index1].x, 100.0f, &controller_msg[11], &controller_msg[12]);
				floatToSignedBytes(target_car_measure[tag_index1].y, 100.0f, &controller_msg[13], &controller_msg[14]);
				floatToSignedBytes(target_car_measure[tag_index1].yaw, 100.0f, &controller_msg[15], &controller_msg[16]);
